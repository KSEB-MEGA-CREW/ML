...
class WebSocketHandler:
    ...
    async def _handle_message(
        ...
        # 타입 에러 디버깅용 로그
        logger.info(f"수신된 메시지 타입: '{message_type}'")
        ...
        # type: None 에러를 해결하기 위한 방어 => type 필드가 없는 메시지는 무시
        if message_type is None:
            logger.warning(f"❌ type 필드가 없는 메시지 무시: {message_data}")
            return
        
        if message_type == MessageType.KEYPOINTS:
            await self._handle_keypoint_data(session_data, message_data)

        elif message_type == MessageType.FRAME_BATCH:
            ...
            logger.warning(f"  > 전체 메시지 데이터: {message_data}")
    ...
    async def _handle_frame_batch(self, session_data: SessionData, message_data: Dict[str, Any]):
        """프레임 배치 처리 (NEW F2T 기능)"""
        try:
            batch_msg = FrameBatchMessage(**message_data)
            
            # 번역이 활성화된 경우에만 프레임 처리
            if not session_data.gloss_collector.is_translation_active():
                logger.debug(
                    f"번역 비활성 상태 - 프레임 배치 무시: {session_data.session_id[:8]}"
                )
                return

            logger.info(
                f"🎬 프레임 배치 수신: {len(batch_msg.frame_batch)}개 (배치 #{batch_msg.batch_index})"
            )

            # 프레임 배치 → 키포인트 배치 변환
            keypoint_batch = await frame_processor.process_frame_batch_async(
                batch_msg.frame_batch
            )

            # 유효한 키포인트가 있는지 확인
            valid_keypoints = [kp for kp in keypoint_batch if kp is not None]
            
            if not valid_keypoints:
                logger.warning(f"프레임 배치에서 유효한 키포인트 추출 실패")
                error_msg = MessageFactory.create_error_message(
                    session_id=session_data.session_id,
                    error_code="KEYPOINT_EXTRACTION_FAILED",
                    error_message="프레임에서 키포인트 추출에 실패했습니다.",
                )
                await session_data.websocket.send_text(error_msg.model_dump_json())
                return

            logger.info(
                f"✅ 키포인트 추출 완료: {len(valid_keypoints)}/{len(keypoint_batch)}개 성공"
            )

            # 모델 추론을 위한 배치 처리
            await self._process_frame_to_text_batch(
                session_data, valid_keypoints, batch_msg.batch_index
            )
            ...
            error_msg = MessageFactory.create_error_message(
                session_id=session_data.session_id,
                error_code="FRAME_BATCH_PROCESSING_ERROR",
                error_message=f"프레임 배치 처리 실패: {str(e)}",
            )
            await session_data.websocket.send_text(error_msg.model_dump_json())

    async def _process_frame_to_text_batch(
        self, 
        ...
        try:
            ...
            error_msg = MessageFactory.create_error_message(
                session_id=session_data.session_id,
                error_code="F2T_BATCH_ERROR",
                error_message=f"F2T 배치 처리 실패: {str(e)}",
            )
            await session_data.websocket.send_text(error_msg.model_dump_json())
    ...
    async def cleanup_all_sessions(self):
        ...
        # FrameProcessor 리소스 정리
        await frame_processor.cleanup()
        
        logger.info("✅ 모든 세션 및 F2T 리소스 정리 완료")
